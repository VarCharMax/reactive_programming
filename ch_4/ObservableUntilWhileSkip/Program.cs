...
namespace ObservableUntilWhileSkip;
...
internal class Program
{
  static void Main(string[] args)
  {
    ...
    takeUntil.Subscribe(value => Console.WriteLine("Until5Seconds: {0}", value));
    ...
    takeWhile.Subscribe(value => Console.WriteLine("WhileSameMinute: {0}", value));

    //skip messages for 5 seconds
    var skipUntil = sourcingSequence.SkipUntil(DateTimeOffset.Now.AddSeconds(5));
    skipUntil.Subscribe(value => Console.WriteLine("SkipFor5Seconds: {0}", value));

    //skip messages of the same minute
    var skipWhile = sourcingSequence.SkipWhile(x => begin.Minute == x.Minute);
    skipWhile.Subscribe(value => Console.WriteLine("SkipSameMinute: {0}", value));
